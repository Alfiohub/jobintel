...
TITLE_RULES: list[tuple[str, str, str, str]] = [
    ...
    (r"\bbusiness analyst\b", "business_analyst", "business_analysis", "business"),
    ...
    (r"\bproduct manager\b", "product_manager", "product_management", "product"),
    (r"\bux designer\b|\bui designer\b|\bproduct designer\b", "product_designer", "design", "design"),
]
...
def _extract_salary(text: str) -> tuple[int | None, int | None, str | None]:
    ...
    for chunk in chunks:
        ...
        for pattern in SALARY_PATTERNS:
            ...
            # Prevent funding-like numbers (e.g. "$130M+") from being treated as salary.
            ...
            s_min = min(vals)
            s_max = max(vals)
            # Sanity guard for annual compensation values.
...
def extract_tags(clean: CleanRow, normalized_title: str) -> dict[str, Any]:
    ...
    location_type = _extract_location_type(text)
    city, region = _split_location(clean.location_clean)
    salary_min, salary_max, salary_currency = _extract_salary(clean.description_clean)
