...
def ensure_table(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS title_onet_crosswalk (
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS ix_title_onet_crosswalk_soc_code ON title_onet_crosswalk(onet_soc_code)"
    )
...
def upsert_row(conn: sqlite3.Connection, row: dict[str, str]) -> bool:
    ...
    onet_soc_code = _s(row.get("onet_soc_code")) or None
    onet_title = _s(row.get("onet_title")) or None
    ...
    conn.execute(
        """
        INSERT INTO title_onet_crosswalk(
          normalized_title,
          onet_soc_code,
          onet_title,
          mapping_confidence,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Load normalized_title -> O*NET crosswalk CSV into SQLite")
