...
_TITLE_META_TOKEN_RE = re.compile(
    r"\b(remote|hybrid|onsite|shift|gmt|utc|ist|pst|cst|est|emea|apac|americas?|"
    r"san francisco|new york|london|berlin)\b",
    re.IGNORECASE,
)

# Order matters: specific patterns first, broad catch-all patterns last.
TITLE_RULES: list[tuple[str, str, str, str]] = [
    ...
    (
        r"(?:\bregional vice president\b.*\b(sales|revenue|commercial|gtm|go[- ]to[- ]market)\b|"
        r"\b(sales|revenue|commercial|gtm|go[- ]to[- ]market)\b.*\bregional vice president\b)",
        "chief_revenue_officer",
        "executive_leadership",
        "business",
    ),
    (r"\bhead of partnerships\b", "alliance_manager", "partnerships", "business"),
    (r"\btreasury analyst\b", "financial_analyst", "finance", "finance"),
    (r"\brecruiting coordinator\b", "technical_recruiter", "recruiting", "hr"),
    (r"\bpartner innovation architect\b", "solutions_architect", "architecture", "engineering"),
    (r"\btechnical author\b", "content_designer", "design", "design"),
    (r"\bvisual designer\b", "product_designer", "design", "design"),
    (r"\bcustomer education\b|\bonboarding\s*(?:&|and)\s*training lead\b|\btraining lead\b", "content_designer", "design", "design"),
    (r"\bdelivery excellence\b", "operations_specialist", "operations", "business"),
    (r"\bthreat intelligence\b", "operations_specialist", "operations", "business"),
    ...
    (r"\bqa automation tester\b|\bqa automation engineer\b", "software_engineer", "software_engineering", "engineering"),
    ...
    (r"\baccounting apprentice\b", "accountant", "finance", "finance"),
    ...
    (r"\bhead of human resources\b", "hr_manager", "hr", "hr"),
    ...
    (r"\bqa engineer\b|\bquality assurance\b", "other", "other", "other"),
    (r"\btechnical support (engineer|specialist|analyst|representative|technician)\b", "it_support_specialist", "it_operations", "engineering"),
...
def _clean_title_for_matching(title: str) -> str:
    ...
    if not cleaned:
        return cleaned
    # Normalize punctuation and separators before metadata trimming.
    cleaned = re.sub(r"[\u2010-\u2015]", "-", cleaned)
    cleaned = re.sub(r"[|/]+", " - ", cleaned)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()

    # Drop bracket blocks that are mostly metadata.
    cleaned = re.sub(
        r"\((?:[^)]*\b(?:remote|hybrid|onsite|shift|gmt|utc|ist|pst|cst|est|emea|apac|americas?)\b[^)]*)\)",
        " ",
        cleaned,
        flags=re.IGNORECASE,
    )
    ...
    if any(_TITLE_META_SUFFIX_RE.search(p) or _TITLE_META_TOKEN_RE.search(p) for p in parts[1:]):
        core = parts[0].strip()
