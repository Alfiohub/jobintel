...
try:
    from experiments.title_context_layer.report_utils import concentration_metrics
except ModuleNotFoundError:  # pragma: no cover - direct script execution
    from report_utils import concentration_metrics


def render_markdown(payload: dict[str, Any], out_path: Path) -> None:
    lines: list[str] = []
    lines.append("# Phase E.15.4 — Partner Lane Shadow Batch v2")
    lines.append("")
...
def run_phase_e15_partner_lane_shadow_batch(
    titled_path: Path,
    promotable_path: Path,
    out_jsonl: Path,
    out_report: Path,
) -> dict[str, Any]:
    promotable_rows = read_jsonl(promotable_path)
    approved_urls = {row["url"] for row in promotable_rows}
    ...
    for row in titled_rows:
        ...
        if row.get("classification_status") == "other" and row.get("url") in approved_urls:
            ...
            out_row["matched_rule_id"] = "ctx_partner_lane_v2"
            out_row["confidence"] = 0.82
            out_row["notes"] = list(out_row.get("notes") or []) + ["phase_e15_partner_lane_shadow_match"]
            matches_added += 1
            rule_hits["ctx_partner_lane_v2"] += 1
    ...
    decision = "partner_lane_shadow_viable_for_review" if matches_added >= 25 else "partner_lane_shadow_needs_more_work"
    reason = (
        "The narrowed partner-only subset produces enough shadow matches to justify production-candidate review."
        if decision == "partner_lane_shadow_viable_for_review"
        else "The narrowed partner-only subset is cleaner, but still too small for the next production-candidate review."
    )
...
if __name__ == "__main__":
    ...
    payload = run_phase_e15_partner_lane_shadow_batch(
        ...
        out_jsonl=root / "data/jobs/jobs_titled_en_recovery_v63_partner_lane_shadow.jsonl",
        out_report=reports / "phase_e15_partner_lane_shadow_batch_v1.json",
    )
    render_markdown(payload, root / "docs/phase_e15_partner_lane_shadow_batch_v1.md")
