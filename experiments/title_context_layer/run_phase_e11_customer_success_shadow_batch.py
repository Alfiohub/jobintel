...
def render_markdown(payload: dict[str, Any], out_path: Path) -> None:
    lines: list[str] = []
    lines.append("# Phase E.11.4 — Customer Success Shadow Batch v1")
    lines.append("")
...
def run_phase_e11_customer_success_shadow_batch(
    titled_path: Path,
    scoring_path: Path,
    out_jsonl: Path,
    out_report: Path,
    ...
    approved_urls = {
        row["url"]
        for row in scoring.get("sample_shadow_matches", [])
        if row.get("normalized_title") == "customer_success_manager"
    }
    ...
    for row in titled_rows:
        ...
        if row.get("classification_status") == "other" and row.get("url") in approved_urls:
            out_row["normalized_title"] = "customer_success_manager"
            out_row["role_family"] = "customer_success"
            out_row["classification_status"] = "matched"
            out_row["match_method"] = "context_shadow_rule"
            out_row["matched_rule_id"] = "ctx_customer_success_lane_v1"
            out_row["confidence"] = 0.8
            out_row["notes"] = list(out_row.get("notes") or []) + ["phase_e11_customer_success_shadow_match"]
            matches_added += 1
            rule_hits["ctx_customer_success_lane_v1"] += 1
    ...
    decision = "customer_success_shadow_lane_viable" if matches_added >= 20 else "customer_success_shadow_lane_needs_more_work"
    reason = (
        "The narrowed customer-success subset produces enough shadow matches to justify formal review for a production-candidate context gate."
        if decision == "customer_success_shadow_lane_viable"
        else "Signal exists, but the narrowed customer-success subset is still too small for the next production-candidate review."
    )
...
if __name__ == "__main__":
    ...
    payload = run_phase_e11_customer_success_shadow_batch(
        titled_path=root / "data/jobs/jobs_titled_en_recovery_v58_recoverability_batch.jsonl",
        scoring_path=reports / "phase_e11_customer_success_scoring_v1.json",
        out_jsonl=root / "data/jobs/jobs_titled_en_recovery_v59_customer_success_shadow.jsonl",
        out_report=reports / "phase_e11_customer_success_shadow_batch_v1.json",
    )
    render_markdown(payload, root / "docs/phase_e11_customer_success_shadow_batch_v1.md")
